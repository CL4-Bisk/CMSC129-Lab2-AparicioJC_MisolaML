...
<div id="messageModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    ...
    <div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <!-- Modal panel -->
        <div
            class="inline-block transform overflow-hidden rounded-lg {{ $colorScheme['bg'] }} {{ $colorScheme['border'] }} border px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6 sm:align-middle">
            <div class="sm:flex sm:items-start">
                <!-- Icon -->
                <div
                    class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full {{ $colorScheme['bg'] }} sm:mx-0 sm:h-10 sm:w-10">
                    @if ($type === 'success')
                        <svg class="h-6 w-6 {{ $colorScheme['icon'] }}" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
