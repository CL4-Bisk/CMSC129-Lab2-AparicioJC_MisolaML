    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <form action="{{ route('borrowed-books.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input id="borrower_name" name="borrower_name" value="{{ old('borrower_name') }}"
                    class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" required />
            </div>

            <div>
                <label class="mb-2 block text-sm font-medium text-gray-700" for="genre">Genre</label>
                <input id="genre" name="genre" value="{{ old('genre') }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-2">
                <button type="submit"
                    class="rounded-md bg-[#1b1b18] px-4 py-2 text-sm font-medium text-white hover:bg-black">Save</button>
                <a href="{{ route('borrowed-books.index') }}"
